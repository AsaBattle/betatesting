...
import FolderOperationModal from '../components/FolderOptionsModal'
...
export default function ViewMode(theUserData) {
  ...
  const [isFolderOperationModalOpen, setIsFolderOperationModalOpen] = useState(false);
  const [currentFolderOperation, setCurrentFolderOperation] = useState('');

  const handleFolderOperation = (operation) => {
    setCurrentFolderOperation(operation);
    setIsFolderOperationModalOpen(true);
  };
  ...
  return (
    <Container maxWidth="false" className={styles.viewMode}>
        ...
        <Grid container spacing={1} alignItems="center">
         ...
         {/*
          <Grid item xs={12} md={4}>
            <Paper elevation={3} className={`${styles.controlPanel} ${styles.compactPaper}`}>
              <FolderOptions
                onNewFolder={() => handleFolderOperation('new')}
                onDeleteFolder={() => handleFolderOperation('delete')}
                onRenameFolder={() => handleFolderOperation('rename')}
                onMoveFolder={() => handleFolderOperation('move')}
              />
            </Paper>
          </Grid>
          <Grid item xs={12} md={4}>
      ...
      <FolderOperationModal
        open={isFolderOperationModalOpen}
        onClose={() => setIsFolderOperationModalOpen(false)}
        operation={currentFolderOperation}
        userId={currentUserId}
      />
    </Container>
  );
